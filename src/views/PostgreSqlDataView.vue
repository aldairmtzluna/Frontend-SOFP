  ...
  <div>
    ...
    <nav id="breadcrumb">
      <ol>
        ...
        <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
        <li><a href="../BPostgreSql">Base de datos PostgreSQL.</a></li>
        <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
    ...
    <div v-if="selectedTable">
      ...
      <div class="button-container" v-if="buttonsVisible">
        <button @click="maskData" class="btn-danger" style="margin-left: 150px; top: 50px; position: relative;">Ofuscar</button>
        <button @click="unmaskData" class="btn-default" style="margin-left: 150px; top: 50px; position: relative;">DesOfuscar</button>
      </div>
    </div>
    <br>
    <br>
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (data) {
        ...
        try {
          const response = await axios.get('http://127.0.0.1:8000/api/postgresql/tablas/');
    ...
    watch(selectedTable, async (newTable) => {
      if (newTable) {
        try {
          const response = await axios.post('http://127.0.0.1:8000/api/postgresql/tablas/datos/', {
            tables: [newTable]
          });
    ...
    const filteredColumns = computed(() => {
      return tableData.value[selectedTable.value as string]?.columns.filter((column: string) => !column.startsWith('original_'));
    });

    const maskData = async () => {
      if (selectedTable.value && selectedColumns.value.length > 0) {
        try {
          await axios.post('http://127.0.0.1:8000/api/postgresql/tablas/enmascarar/', {
    ...
    const unmaskData = async () => {
      if (selectedTable.value && selectedColumns.value.length > 0) {
        try {
          await axios.post('http://127.0.0.1:8000/api/postgresql/tablas/desenmascarar/', {
            table: selectedTable.value,
            columns: selectedColumns.value
          });
