  ...
  <div>
    ...
    <nav id="breadcrumb">
      <ol>
        ...
        <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
        <li class="pag">Bases de Datos PostgreSQL</li>
      </ol>
    </nav>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(baseDatos, index) in basesDatosFiltradas" :key="baseDatos.id">
            ...
            <td class="acciones">
              <a :href="'../editar-postgresql/' + baseDatos.id">
...
export default defineComponent({
  setup() {
    ...
    const cargarDatos = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/bases-postgresql/');
    ...
    const cargarConexion = async (idConexion) => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/cargar-postgresql/${idConexion}/`);
    ...
    const enviarFormulario = async () => {
      try {
        const response = await axios.post('http://127.0.0.1:8000/api/postgresql/', conexionDetalle.value);
        ...
        router.push({ name: 'postgresql-datos' });
      } catch (error) {
        console.error(error);
        Swal.fire({
          position: "center",
    ...
    const confirmarEliminacion = (baseDatos) => {
      Swal.fire({
        ...
        // text: `Eliminarás la conexión: ${baseDatos.nombre}`,
        icon: 'warning',
    ...
    const eliminarConexion = async (idConexion) => {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/eliminar-postgresql/${idConexion}/`);
        Swal.fire({
          position: "center",
          icon: "success",
    ...
    return { basesDatosFiltradas, filtro, buscar, abrirModal, cerrarModal, modalActivo, conexionDetalle, enviarFormulario, confirmarEliminacion };
  }
});
</script>
