    ...
    <div>
      ...
      <nav id="breadcrumb">
        <ol>
          ...
          <li class="pag"><a href="../BPostgreSql">Conexiones</a></li>
          <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
      ...
      <div>
        <h1>Editar Conexión PostgreSQL</h1>
...
<br><br>
        <div class="row">
          <div class="col-md-3">
            <a href="../BPostgreSql"><button class="btn btn-default pull-right" type="button">Regresar</button></a>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async obtenerConexion() {
      ...
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/cargar-postgresql/${conexionId}/`);
    ...
    async guardarCambios() {
      try {
        await axios.put(`http://127.0.0.1:8000/api/cargar-postgresql/${this.conexion.id_conexion}/`, this.conexion);
        
        this.$router.push({ name: 'BPostgreSql' }); 
      } catch (error) {
