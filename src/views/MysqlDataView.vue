  ...
  <div>
    ...
    <nav id="breadcrumb">
      <ol>
        ...
        <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
        <li><a href="../BMySql">Bases de Datos MySql.</a></li>
        <li><span class="icons icon-arrow" aria-hidden="true"></span></li>
    ...
    <div v-if="selectedTable">
      ...
      <div class="button-container" v-if="buttonsVisible">
        <button @click="maskData" class="btn-danger" style="margin-left: 20px; top: 50px; position: relative;">Ofuscar</button>
        <button @click="unmaskData" class="btn-default" style="margin-left: 20px; top: 50px; position: relative;">DesOfuscar</button>
        <button @click="viewObfuscatedData" class="btn-primary" style="margin-left: 20px; top: 50px; position: relative;">Ver datos ofuscados</button>
      </div>
    </div>
    <br>
    <br>
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedTable = ref<string>(''); // Cambiado a string vacío
    ...
    onMounted(async () => {
      ...
      if (data) {
        ...
        try {
          const response = await axios.get('http://127.0.0.1:8000/api/mysql/tablas/');
    ...
    watch(selectedTable, async (newTable) => {
      if (newTable) {
        try {
          const response = await axios.post('http://127.0.0.1:8000/api/mysql/tablas/datos/', {
            tables: [newTable]
          });
    ...
    const filteredColumns = computed(() => {
      return tableData.value[selectedTable.value]?.columns.filter((column: string) => !column.startsWith('original_'));
    });

    const maskData = async () => {
      if (selectedTable.value && selectedColumns.value.length > 0) {
        try {
          await axios.post('http://127.0.0.1:8000/api/mysql/tablas/enmascarar/', {
    ...
    const unmaskData = async () => {
      if (selectedTable.value && selectedColumns.value.length > 0) {
        try {
          await axios.post('http://127.0.0.1:8000/api/mysql/tablas/desenmascarar/', {
            table: selectedTable.value,
            columns: selectedColumns.value
          });
    ...
    const viewObfuscatedData = () => {
      window.location.href = '/datos-ofuscados';
    };
